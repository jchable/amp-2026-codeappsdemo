...
export function TicketList({
  tickets, onChangerStatut, onSupprimer,
}: {
  tickets: Ticket[];
      ...
      {tickets.map((t) => (
        <li key={t.id} className={"carte ticket p-" + t.priorite.toLowerCase()}>
          ...
          <div className="actions">
            <select value={t.statut} onChange={(e) => onChangerStatut(t.id, e.target.value as Statut)} aria-label={"Statut de " + t.titre}>
              {STATUTS.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
            <button className="lien" onClick={() => onSupprimer(t.id)}>Supprimer</button>
          </div>
        </li>
      ))}
    </ul>
